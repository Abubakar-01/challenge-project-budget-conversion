...
// TTD conversion for specific projects
endpoints.post('/api-conversion', handleTtdConversion)
...
async function handleUpdateProject (req, res) {
  try {
    const validation = validateProjectData({ ...req.body, projectId: parseInt(req.params.id) })
    if (!validation.valid) {
      return res.status(400).json(formatResponse(false, null, validation.error))
    }

    await updateProject(req.params.id, req.body)
    res.json(formatResponse(true, { message: 'Project updated successfully' }))
  } catch (error) {
    const { status, message } = handleError(error)
    res.status(status).json(formatResponse(false, null, message))
  }
...
async function handleDeleteProject (req, res) {
  try {
    ...
    res.json(formatResponse(true, { message: 'Project deleted successfully' }))
  } catch (error) {
    const { status, message } = handleError(error)
    res.status(status).json(formatResponse(false, null, message))
  }
...
async function handleTtdConversion (req, res) {
  ...
  try {
    const { projectName, year = new Date().getFullYear() } = req.body
    ...
    if (!allowedProjects.includes(projectName)) {
      return res.status(400).json(formatResponse(false, null, 'Project not available for TTD conversion'))
    }
    ...
    const convertedProjects = await convertProjectsToCurrency(projects, 'TTD')
    res.json(formatResponse(true, convertedProjects))
  } catch (error) {
    const { status, message } = handleError(error)
    res.status(status).json(formatResponse(false, null, message))
...
async function updateProject (id, projectData) {
  ...
  const params = [
    projectData.projectName, projectData.year, projectData.currency,
    projectData.initialBudgetLocal, projectData.budgetUsd,
    projectData.initialScheduleEstimateMonths, projectData.adjustedScheduleEstimateMonths,
    projectData.contingencyRate, projectData.escalationRate,
    projectData.finalBudgetUsd, id
  ]
